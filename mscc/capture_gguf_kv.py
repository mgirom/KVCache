...
print(f"parsed {n_cells} cells x {len(geo['kv_layers'])} KV layers of {geo['n_layer']}; K row norm mean {float(states[(0,'k')].norm(dim=1).mean()):.2f}", flush=True)
H, d = geo["n_head_kv"], geo["head_dim"]
bph = a.codes * GGML_BLOCK_BYTES[a.quant] * 8 // 32
books = K.fit_kv_codebooks_perhead(states, bph, H, d, quant=a.quant, codes=a.codes,
...
meta = {"model": os.path.basename(a.gguf), "arch": geo["arch"], "basis": "postrope", "per_head": True,
        "quant": a.quant, "codes_per_head": a.codes, "bits_per_head": bph, "unit_bits": H * bph,
...
os.system(f"{sys.executable} {os.path.join(HERE, 'export_cpca_gguf.py')} {a.out} --n-head {geo['n_head']} -o {out_gguf}")
